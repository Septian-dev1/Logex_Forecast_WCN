...
fc_best = fc_q.groupby(['specialism_code', 'ds'], group_keys=False).apply(pick_best)
print("Columns before reset_index:", fc_best.columns)
fc_best = fc_best.reset_index(drop=True)
print("Columns after reset_index:", fc_best.columns)
fc_best = fc_best.rename(columns={'yhat': 'vol_spec_total'})
print("Columns after rename:", fc_best.columns)
